...
public class CIMS_GCP_NewQuery {
	...
	public boolean GCP_NewQuery_Initiation_Page(String filename, String sheetName,int ColumnCounter,String mode)throws AWTException, InterruptedException {
			...
			// String sCoordinator                                =           UtilFunction.getCellData(filename, sheetName, 9, ColumnCounter);
			 String GCPEmployeeID                               =           UtilFunction.getCellData(filename, sheetName, 9, ColumnCounter);
			 String GCPNationalityCountry                       =           UtilFunction.getCellData(filename, sheetName, 10, ColumnCounter);
			 
			 String GCPSPecialVisa                              =           UtilFunction.getCellData(filename, sheetName, 11, ColumnCounter);
			 String GCPSPecialVisaPermit                        =           UtilFunction.getCellData(filename, sheetName, 12, ColumnCounter);
			 
			 String GCPDestinationOriginCountryCode             =           UtilFunction.getCellData(filename, sheetName, 13, ColumnCounter);
			 String GCPDestinationCountryCode                   =           UtilFunction.getCellData(filename, sheetName, 14, ColumnCounter);
			 
			 String GCPDestinationCity                          =           UtilFunction.getCellData(filename, sheetName, 15, ColumnCounter);
			 String form_PurposeCategory                          =           UtilFunction.getCellData(filename, sheetName, 16, ColumnCounter);
			...
			if(TestcaseRunMode.equals("Y")){
				...
				String SelectRecordXpath =".//*[@id='divContactListPartial']//*[@class='table table-item-list']//tbody//tr[1]//td[5]//*[text()='Select']";
...
//						}
						...
						try {
		            	 ...
		            	 System.out.println("RecordNotFound is" +RecordNotFound);
		            	 String CancelBtnXpath =".//*[contains(@class,'close') and text()='×']";
		            	 if(RecordNotFound.equalsIgnoreCase("No Results Found.")){
		            		 ...
		            		 try {
								...
								System.out.println("Unable to click on the cancel button");
							}  
		            		Flag=false;
						...
						Thread.sleep(2000);
						 ...
						 String Country_CodeNewXpath="//*[@id='form1']//*[contains(@class,'control-group')][4]//*[contains(@class,'controls')]//select[@name]";
							...
							String CountryvisapermitDropDownXpath=".//*[@id='SpecialConditionCountry']";
							String visapermitDropDownXpath=".//*[@id='SpecialCondition']";
							String AddVisaPrmitXpath="//*[contains(@class,'FieldValue')]//*[contains(@class,'btn') and text()='Add']";
							
							try {
								Thread.sleep(1000);
								utilfunc.Selectdropdownvaluebytext(CountryvisapermitDropDownXpath, GCPSPecialVisa);
								Thread.sleep(1000);
							} catch (Exception e1) {
							...
							try {
								Thread.sleep(1000);
								utilfunc.Selectdropdownvaluebytext(visapermitDropDownXpath, GCPSPecialVisaPermit);
								Thread.sleep(1000);	
 							} catch (Exception e1) {
							...
							try {
								Thread.sleep(1000);
								utilfunc.MakeElement(AddVisaPrmitXpath).click();
								Thread.sleep(1000);
							} catch (Exception e1) {
							...
							for(int k=1; k<=resultcountDestation; k++){
								...
								if(AttributeName.equals("form_OriginCountryCode")){
									...
									utilfunc.Selectdropdownvaluebytext(NewDestination, GCPDestinationOriginCountryCode);
                                    ...
                                    else if(AttributeName.equals("form_PurposeCategory")){
									
									utilfunc.Selectdropdownvaluebytext(NewDestination, form_PurposeCategory);
								}
									...
									try {
										...
										System.out.println("Unable to add the purpose");
									}
								}
